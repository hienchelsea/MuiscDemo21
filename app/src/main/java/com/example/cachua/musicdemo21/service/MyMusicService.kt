package com.example.cachua.musicdemo21.service
...
import com.example.cachua.musicdemo21.ui.music.ui.music.MusicActivity
...
class MyMusicService : Service() {
    ...
    fun chooseMusic(context: Context, musics: ArrayList<MusicModel>, position: Int) {
    ...
    fun updateMusicTime(music: MusicModel, progress: Int, position: Int) {
        Handler().postDelayed({
            ...
            if (progress == 0 && serviceStatus == 1) callBack.onUpTime(music, mediaPlayer.currentPosition)
    ...
    private fun pushNotification(music: MusicModel) {
        ...
        val pendingIntent = PendingIntent.getActivity(this, 0, intentNotification, PendingIntent.FLAG_UPDATE_CURRENT)

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val importance = NotificationManager.IMPORTANCE_DEFAULT
            val notificationChannel = NotificationChannel(Constant.CHANNEL_ID, Constant.CHANNEL_NAME, importance)
    ...
    private fun getNotification(music: MusicModel, pendingIntent: PendingIntent, flags: Int): Notification {
        ...
        notificationLayout.run {
            ...
            setImageViewResource(R.id.imageAvatarPlay, R.drawable.ic_music_player)
            if (mediaPlayer.isPlaying) {
                setImageViewResource(R.id.imagePlay, R.drawable.ic_pause_two)
            } else {
                setImageViewResource(R.id.imagePlay, R.drawable.ic_play_two)
            }
        }
